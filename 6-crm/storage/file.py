...
def load() -> tuple[List[Task], int]:
    ...
    try:
        ...
        try:
            ...
            # Сразу пересохраняем в правильной кодировке
            save(tasks)  # пока tasks пустой, но потом перезапишет нормально
        except Exception as e:
        ...
        # Если всё ок — продолжаем парсить
        pass

    # Парсинг задач (одинаковый в обоих случаях)
    ...
    # После успешной загрузки (даже если была cp1251) — пересохраняем в UTF-8
    if any(t["title"] for t in tasks):  # если хоть одна задача есть
        save(tasks)
